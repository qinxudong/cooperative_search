#coding=utf-8
from classAgent_multi import Agent
import numpy as np
import pandas as pd
...
#输入搜索区域矩阵数据
df = pd.read_csv('/home/qin/github/multiagent_targetsearch-qin/input2.csv', header=None)
matrix_sr = df.values
'''
fig_input = plt.figure(figsize=(8, 8))
ax_input = fig_input.add_subplot(111)
map_input = matrix_sr
im_input = ax_input.imshow(map_input, cmap='bone_r', vmin=0, vmax=1)
'''
...
q = 0.3
radius_search = 4
radius_communicate = 8
Ku = 1
bound_speed = 4
bound_Q = 10000000000000000

#agent初始化
agent0 = Agent(0, coords[0], map_init, p, q, radius_search, radius_communicate, Ku, bound_speed,  bound_Q)
agent1 = Agent(1, coords[1], map_init, p, q, radius_search, radius_communicate,Ku, bound_speed,  bound_Q)
agent2 = Agent(2, coords[2], map_init, p, q, radius_search, radius_communicate, Ku,bound_speed,  bound_Q)
agent3 = Agent(3, coords[3], map_init, p, q, radius_search, radius_communicate, Ku,bound_speed,  bound_Q)
agent4 = Agent(4, coords[4], map_init, p, q, radius_search, radius_communicate, Ku,bound_speed,  bound_Q)
agent5 = Agent(5, coords[5], map_init, p, q, radius_search, radius_communicate, Ku,bound_speed,  bound_Q)
...
for i in range(N):
    ax.plot(coord_pls[i][0], coord_pls[i][1], color=colors[i], marker='>', linewidth=0.3)
plt.pause(0.1)
